import torch
import os
from agent.base_net import RNN
from agent.vdn_net import VDNNet
...
class VDN:
	def __init__(self, config, state_shape):
		...
		if config.marl.reuse_network:
			input_shape += self.n_agents

		# 神经网络
		self.eval_rnn = RNN(input_shape, config)  # 每个agent选动作的网络
		self.target_rnn = RNN(input_shape, config)
		...
		self.model_dir = config.marl.model_dir
		# 如果存在模型则加载模型
		if self.config.marl.load_model:
		...
		self.optimizer = torch.optim.RMSprop(self.eval_parameters, lr=config.marl.lr)
	...
	def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
		...
		mask = torch.tensor([1])  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习
		if torch.cuda.is_available():
			u = u.cuda()
			r = r.cuda()
			mask = mask.cuda()
		...
		targets = r + self.config.marl.gamma * q_total_target
		...
		loss.backward()
		torch.nn.utils.clip_grad_norm_(self.eval_parameters, self.config.marl.grad_norm_clip)
		self.optimizer.step()

		if train_step > 0 and train_step % self.config.marl.target_update_cycle == 0:
			self.target_rnn.load_state_dict(self.eval_rnn.state_dict())
			self.target_vdn_net.load_state_dict(self.eval_vdn_net.state_dict())

	def _get_inputs(self, batch, transition_idx):
		# 取出所有episode上该transition_idx的经验，u_onehot要取出所有，因为要用到上一条
		state, state_next, u_onehot = batch['s'][:, transition_idx], \
								  batch['s_next'][:, transition_idx], batch['u_onehot'][:]
		episode_num = state.shape[0]
		inputs, inputs_next = [], []
		inputs.append(state)
		...
		if self.config.marl.reuse_network:
			...
			inputs.append(torch.eye(self.config.marl.n_agents).unsqueeze(0).expand(episode_num, -1, -1))
			inputs_next.append(torch.eye(self.config.marl.n_agents).unsqueeze(0).expand(episode_num, -1, -1))
		# 要把obs中的三个拼起来，并且要把episode_num个episode、self.args.n_agents个agent的数据拼成episode_num*n_agents条数据
		# 因为这里所有agent共享一个神经网络，每条数据中带上了自己的编号，所以还是自己的数据
		inputs = torch.cat([x.reshape(episode_num * self.config.marl.n_agents, -1) for x in inputs], dim=1)
		inputs_next = torch.cat([x.reshape(episode_num * self.config.marl.n_agents, -1) for x in inputs_next], dim=1)
	...
	def init_hidden(self, episode_num):
		...
		self.eval_hidden = torch.zeros((episode_num, self.n_agents, self.config.marl.hidden_dim))
		self.target_hidden = torch.zeros((episode_num, self.n_agents, self.config.marl.hidden_dim))
